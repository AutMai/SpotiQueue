      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <ConfigItem
            config={config}
            updateConfig={updateConfig}
            label={<label className="flex items-center gap-2"><input type="checkbox" checked={config.rooms_enabled !== 'false'} onChange={(e) => handleChange('rooms_enabled', e.target.checked ? 'true' : 'false')} /> Require a room code</label>}
            saveKey="rooms_enabled"
